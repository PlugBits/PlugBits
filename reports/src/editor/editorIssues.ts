...
const buildRequiredIssue = (rule: SlotRule, severity: IssueSeverity): Issue => ({
  ...
  group: rule.group,
  label: rule.label,
  message: `『${rule.label}』のフィールドが未設定です。右側の一覧から選んでください。`,
});
...
const buildEmptyValueIssue = (rule: SlotRule, severity: IssueSeverity): Issue => ({
  ...
  group: rule.group,
  label: rule.label,
  message: `『${rule.label}』が空です。レコードに値を入力するか、別フィールドに変更してください。`,
});
...
const buildTableRequiredIssue = (table: TableRule): Issue => ({
  ...
  group: table.group,
  label: table.label,
  message: `『${table.label}』のサブテーブルが未設定です。`,
});
...
const buildTableEmptyIssue = (table: TableRule): Issue => ({
  ...
  group: table.group,
  label: table.label,
  message: '明細が0件です。サブテーブル（明細）に1行以上追加してください。',
});
...
const buildColumnRequiredIssue = (table: TableRule, column: ColumnRule, severity: IssueSeverity): Issue => ({
  ...
  group: table.group,
  label: column.label,
  message: `『${column.label}』のフィールドが未設定です。右側の一覧から選んでください。`,
});
...
const buildColumnEmptyIssue = (table: TableRule, column: ColumnRule): Issue => ({
  ...
  group: table.group,
  label: column.label,
  message: `明細の『${column.label}』が空の行があります。空欄を埋めてください。`,
});
