...
# 追記(2026-02-25): Saved/Current draft プレビュー手順

手動テスト(5分)
1. テンプレ編集画面で Preview source = Current draft にして PDFプレビュー (debug=1)
2. Preview source = Saved template (KV) に切替えて PDFプレビュー (debug=1)
3. どちらも /render が 200 で、worker log に DBG_RENDER_RESOLVED が出る
4. saved/record の resolvedSource が userTemplate になり、record出力と見た目が一致する
   (必要なら resolvedTemplateHash を比較)

期待
- Saved template の payload に kintone.baseUrl / kintone.appId が含まれる
- Missing kintone.* の 400 が発生しない
