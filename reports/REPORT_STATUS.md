...
### 3. kintone プラグイン
- 設定画面（APIベースURL、APIキー、テンプレID、添付フィールド）を実装。
...
## ロードマップ
1. **プラグイン仕上げ**
   - 設定未入力時の警告、進行中インジケータ、トーストなど UX 強化。
   - Worker 連携の認証方式（kintone APIトークンと Worker APIキーの整理）とエラー処理を改善。
   - README/PLUGIN_DESIGN.md を拡張し、導入・設定手順を明文化。
